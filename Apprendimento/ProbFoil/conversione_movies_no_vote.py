#------------------------------------------------------------
# Converte il dataset movies_final.csv in un file prolog
# escludendo l'attributo vote_average, dato che è il target del training
#------------------------------------------------------------
...
def csv_to_prolog(csv_file, prolog_file):
    ...
    with open(prolog_file, 'w', encoding='utf-8') as f:
        ...
        for index, row in df.iterrows():
            ...
            f.write(f'film({budget}, \"{genres}\", {id}, \"{original_language}\", {popularity}, \"{production_companies}\", \"{release_date}\", {revenue}, {runtime}, \"{title}\", {vote_count}).\n')

csv_to_prolog('Data preparation/Dataset finali/movies_final.csv', 'Apprendimento/ProbFoil/movies_new.pl')
